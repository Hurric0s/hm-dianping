...
public class RedisCacheHelper {
    ...
    public <R,ID>void rebuildCache(ID id,Function<ID,R>dbFallback,String keyPrefix ,Long time, TimeUnit unit) {
        // 1. 查询数据库
        R r = dbFallback.apply(id);
        String key=keyPrefix + id;
        this.setWithLogicalExpire(key, r, time, unit);// 2. 写入redis，带逻辑过期时间
    }
